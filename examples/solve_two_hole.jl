...
function solve_case(; mode, d, R1 = 1.0, R2 = 100.0, M = 4, p = 6)
    p1 = (d / 2, 0.0)
    p2 = (-d / 2, 0.0)
    ...
    return (; mesh, refel, dof, u, uex, ndofs = dof.ndofs, t_solve, linf)
end
...
function main()
    ...
    phase("Summary")
    @printf("  %-24s %9s %10s %13s\n", "setup", "DOFs", "solve [s]", "L∞ error")
    for (name, r) in (("well-separated (d=10)", res_sep), ("close (d=4)", res_tou))
        @printf("  %-24s %9d %10.3f %13.3e\n", name, r.ndofs, r.t_solve, r.linf)
    end
